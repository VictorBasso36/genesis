...
function ConvertWhatsapp() {
  ...
  useEffect(() => {
    axios
      .get('https://api.grupogenesis.com.br/api/links-whats-apps?populate[LinksWhatsapp][populate]=*&populate[Telefones][populate]=*')
      .then((response) => {
  ...
  useEffect(() => {
    if (dataLinks.length > 0) {
      const randomIndex = Math.floor(Math.random() * dataLinks.length);
      const randomLinksWhatsapp = dataLinks[randomIndex].attributes.LinksWhatsapp;
      const randomLinkIndex = Math.floor(Math.random() * randomLinksWhatsapp.length);
      setRandomLink(randomLinksWhatsapp[randomLinkIndex].Link);
    }
  }, [dataLinks]);

  return (
    randomLink && (
      <a href={randomLink} className="linkconvert" title="Dúvidas sobre seu apartamento?" target="_blank">
